...
import { BadgeCheck, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, PiggyBank, Star, Zap } from 'lucide-react';
...
export default function CustomerTestimonials() {
    ...
    const stats = [
        { icon: <CheckCircle2 size={28} className="material-symbols-outlined text-primary text-3xl" />, label: t('Stats.installations'), value: "2,000+" },
        { icon: <Zap size={28} className="material-symbols-outlined text-primary text-3xl" />, label: t('Stats.energy'), value: "50MW" },
        { icon: <PiggyBank size={28} className="material-symbols-outlined text-primary text-3xl" />, label: t('Stats.savings'), value: "$5M+" }
    ];
    ...
    // Chunk testimonials for carousel (3 per slide on desktop)
    const chunkedReviews = [];
    for (let i = 0; i < filteredTestimonials.length; i += 3) {
        chunkedReviews.push(filteredTestimonials.slice(i, i + 3));
    }

    // Reset reviewsIndex when filter changes
    useEffect(() => {
        setReviewsIndex(0);
    }, [activeFilter]);
            ...
            <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden bg-background-light dark:bg-background-dark text-[#111811] dark:text-white font-display">
                ...
                <main className="flex h-full grow flex-col items-center">
                                    ...
                                    >
                                        <div className="flex flex-col gap-4 text-center z-10 max-w-3xl">
                                            <div className="flex items-center justify-center gap-2 mb-2">
                                                ...
                                                <span className="text-white font-medium">{t('Hero.ratingInfo')}</span>
                                            </div>
                    ...
                    <div className="w-full px-4 md:px-10 lg:px-40 py-2 flex justify-center">
                        <div className="flex flex-col max-w-[960px] flex-1 px-4">
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 pt-5 border-b border-[#f0f4f0] dark:border-gray-800">
                                <div>
                                    <span className="text-primary font-bold tracking-wider uppercase text-sm mb-2 block">{t('Stories.badge')}</span>
                                    <h2 className="text-[#111811] dark:text-white tracking-light text-[28px] font-bold leading-tight text-left">{t('Stories.title')}</h2>
                                </div>

                                <div className="flex gap-2">
                                    <button
                                        onClick={() => setCarouselIndex((prev) => (prev - 1 + featuredStories.length) % featuredStories.length)}
                                        className="p-2 rounded-full cursor-pointer border border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
                                    >
                                        <ChevronLeft />
                                    </button>
                                    <button
                                            ...
                                            <div className="flex w-full @xl:w-1/2 grow flex-col items-stretch justify-center gap-1 py-6 px-6 relative bg-white dark:bg-[#1a2e1a] z-10">
                                                ...
                                                <div className="flex flex-col gap-4 mt-2">
                                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                                        {story.details.map((detail: { label: string; value: string }, idx: number) => (
                    ...
                    <div className="w-full px-4 md:px-10 lg:px-40 py-10 bg-[#f9fafb] dark:bg-[#0c1a0c] flex justify-center border-t border-[#e5e7eb] dark:border-gray-800">
                        <div className="flex flex-col max-w-[960px] flex-1">
                            {/* Filter Bar */}
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 pt-5 border-b border-[#f0f4f0] dark:border-gray-800 px-4">
                                <div>
                                    <h2 className="text-[#111811] dark:text-white text-2xl font-bold">{t('Reviews.title')}</h2>
                                </div>

                                <div className="flex gap-2">
                                    <button
                                        onClick={() => setReviewsIndex((prev) => (prev - 1 + chunkedReviews.length) % chunkedReviews.length)}
                                        className="p-2 rounded-full cursor-pointer border border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
                                    >
                                        <ChevronLeft />
                                    </button>
                                    <button
                                        onClick={() => setReviewsIndex((prev) => (prev + 1) % chunkedReviews.length)}
                                        className="p-2 rounded-full cursor-pointer border border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
                                    >
                                        <ChevronRight />
                                    </button>
                            ...
                            <div className="px-4">
                                <Carousel
                                    items={chunkedReviews}
                                    index={reviewsIndex}
                                    onChange={setReviewsIndex}
                                    minHeight="300px"
                                    renderItem={(chunk) => (
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-4">
                                            {chunk.map((testimonial: ITestimonial) => (
                                                <div key={testimonial.id} className="flex flex-col justify-between gap-4 rounded-xl border border-[#e5e7eb] bg-white p-6 shadow-sm hover:shadow-md transition-shadow dark:bg-[#1a2e1a] dark:border-gray-700 h-full">
                                                    <div>
