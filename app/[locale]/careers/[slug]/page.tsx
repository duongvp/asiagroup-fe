...
export default function JobDetail() {
    ...
    // Trong thực tế, dữ liệu này có thể fetch từ API dựa trên ID
    const jobData = {
    ...
    return (
        <>
            <Head>
                <title>{jobData.title} - SolarTech Careers</title>
                            ...
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-6 text-[#618961] dark:text-gray-400">
                                        <span className="flex items-center gap-1.5"><MapPin size={18} /> {jobData.location}</span>
                                        <span className="flex items-center gap-1.5"><Banknote size={18} /> {jobData.salary}</span>
                                        <span className="flex items-center gap-1.5"><Clock size={18} /> {t('JobDetail.posted')} {jobData.posted}</span>
                                    </div>
                                </div>
                                <a
                                    href="#apply"
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                                ...
                                <div className="lg:col-span-2 space-y-12">
                                    <div>
                                        ...
                                        <div className="text-[#618961] dark:text-gray-400 leading-relaxed space-y-4">
                                            <p>{t('JobDetail.description.p1')}</p>
                                            <p>{t('JobDetail.description.p2')}</p>
                                        </div>
                                    </div>
                                        ...
                                        <ul className="space-y-4">
                                            {[0, 1, 2, 3, 4].map((i) => (
                                                <li key={i} className="flex gap-4">
                                                    ...
                                                    <span className="text-[#618961] dark:text-gray-400 leading-relaxed">
                                                        {t(`JobDetail.responsibilitiesItems.${i}`)}
                                                    </span>
                                                </li>
                                            ))}
                                        </ul>
                                        ...
                                        <ul className="space-y-4 text-sm text-[#618961] dark:text-gray-400">
                                            {[0, 1, 2, 3, 4].map((i) => (
                                                <li key={i} className="flex items-start gap-3">
                                                    ...
                                                    {t(`JobDetail.requirementsItems.${i}`)}
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                        ...
                        <div className="max-w-[800px] mx-auto">
                            <div className="bg-white dark:bg-[#152615] rounded-3xl p-8 md:p-12 shadow-sm border border-[#dbe6db] dark:border-[#1a2e1a]">
                                ...
                                <form className="space-y-6">
                                    ...
                                    <button className="bg-primary hover:bg-green-400 text-[#111811] text-lg font-bold h-14 w-full rounded-xl transition-all shadow-lg flex items-center justify-center gap-2">
